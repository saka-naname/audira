...
pub async fn scan_library(pool: &SqlitePool, base_dir: &str) -> Result<(), LibraryScanError> {
    ...
    for (file, hash) in files {
        ...
        let Some((song, song_metadata)) = extract_song_params(file, hash) else {
            continue;
        };

        let (song, _) = songs_repository::insert_with_metadata(&mut tx, &song, &song_metadata)
            .await
            .map_err(|_| LibraryScanError::WriteTransactionFailed)?;

        create_or_connect_album(&mut tx, song).await?;
...
fn extract_song_params(
    ...
    songs_repository::InsertSongsParams,
    songs_repository::InsertSongMetadataParams,
)> {
...
async fn create_or_connect_album(
    ...
    if let Some(album) = album {
        ...
        let album = insert_album(&mut *conn, &params)
            .await
